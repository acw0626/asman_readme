...
기능적 요구사항
1. 고객이 필요한 자율운행 수리기사등급을 선택하고 호출 요청한다.
2. 고객 위치에서 가용 수리기사를 조회 후 수리기사를 할당 요청한다.
...
비기능적 요구사항
1. 트랜잭션
- 수리기사 할당확인 되지 않으면 고객 호출요청을 할 수 없다. Sync 호출
2. 장애격리
...
2) 호출요청 완료시 할당확인 및 결과 전송: Taxi manage service 에서taxi Assign 마이크로서비스로 수리기사할당 요청이 전달되는 과정에 있어서 
...
## DDD 의 적용
총 3개의 Domain 으로 관리되고 있으며, 수리기사요청(Taxicall) , 수리기사관리(TaxiManage), 수리기사할당(TaxiAssign) 으로 구성된다. 
...
- taxicall 서비스 호출처리
호출(taxicall)->수리기사관리(taximanage) 간의 호출처리 됨.
수리기사 할당에서 수리기사기사를 할당하여 호출 확정 상태가 됨.
두 개의 호출 상태
를 만듬.
```
http localhost:8081/수리기사호출s 휴대폰번호="01012345678" 호출상태=호출 호출위치="마포" 예상요금=25000
http localhost:8081/수리기사호출s 휴대폰번호="01056789012" 호출상태=호출 호출위치="서대문구" 예상요금=30000
```

![image](screenshots/taxicall1.png "taxicall 서비스 호출")
![image](screenshots/taxicall2.png "taxicall 서비스 호출")

호출 결과는 모두 수리기사 할당(taxiassign)에서 수리기사기사의 할당으로 처리되어 호출 확정 상태가 되어 있음.

![image](screenshots/taxicall_result1.png "taxicall 서비스 호출 결과")
![image](screenshots/taxicall_result2.png "taxicall 서비스 호출 결과")
![image](screenshots/taximanage_result1.png "taxicall 서비스 호출 결과 - 수리기사관리")


- taxicall 서비스 호출 취소 처리

호출 취소는 수리기사호출에서 다음과 같이 호출 하나를 취소 함으로써 진행 함.

```
http delete http://localhost:8081/수리기사호출s/1
HTTP/1.1 204
Date: Tue, 02 Mar 2021 16:59:12 GMT
```
호출이 취소 되면 수리기사 호출이 하나가 삭제 되었고, 

```
http localhost:8081/수리기사호출s/
```
...
호출(taxicall)->수리기사관리(taximanage) 간의 호출은 동기식 일관성을 유지하는 트랜잭션으로 처리함.
호출 프로토콜은 이미 앞서 Rest Repository 에 의해 노출되어있는 REST 서비스를 FeignClient 를 이용하여 호출하도록 한다. 


```
# external > 수리기사관리Service.java
...
@FeignClient(name="taximanage", url="http://localhost:8082", fallback = 수리기사관리ServiceFallback.class)
...
```
# external > 수리기사관리ServiceFallback.java
...
```
# 수리기사 관리 시스템 down 후 taxicall 호출 

#taxicall

C:\Users\Administrator>http localhost:8081/수리기사호출s 휴대폰번호="01012345678" 호출상태="호출"
```

![수리기사관리죽으면수리기사콜놉](https://user-images.githubusercontent.com/78134019/109464780-905d6180-7aaa-11eb-9c90-e7d1326deea1.jpg)

```
# 수리기사 관리 (taximanage) 재기동 후 주문하기

#주문하기(order)
http localhost:8081/수리기사호출s 휴대폰번호="01012345678" 호출상태="호출"
```

![수리기사관리재시작](https://user-images.githubusercontent.com/78134019/109464984-e5997300-7aaa-11eb-9363-b7bfe15de105.jpg)
...
수리기사 관리 (Taxi manage) 이후 수리기사 할당(Taxi Assign) 은 비동기식 처리이므로 , 수리기사 호출(Taxi call) 의 서비스 호출에는 영향이 없다
 
고객이 수리기사 호출(Taxi call) 후 상태가 [호출]->[호출중] 로 변경되고 할당이 완료되면 [호출확정] 로 변경이 되지만 , 수리기사 할당(Taxi Assign)이 정상적이지 않으므로 [호출중]로 남아있음. 
--> (시간적 디커플링)
<고객 수리기사 호출 Taxi call>
...
![수리기사호출_수리기사할당없이_조회](https://user-images.githubusercontent.com/78134019/109471791-99ebc700-7ab4-11eb-924f-03715de42eba.png)
